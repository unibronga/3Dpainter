...
| Сохранить проект / как | **Cmd+S** / **Cmd+Shift+S** |
...
**Эффекты.** Меню **Эффекты** меняет то, как модель показана, а не саму
покраску. **Пиксель-арт** рисует модель крупными квадратными пикселями,
с урезанной палитрой и тёмным контуром по силуэту на выбор. **Аниме** —
cel-shading в духе Genshin Impact: свет ступенями с резкой прохладной тенью,
чернильная линия по силуэту и сгибам, светлая кромка по краю. Кольцо кисти и сетка пола остаются тонкими — красить с эффектом можно,
а **Вид в PNG** снимает картинку с эффектом.

**Свой формат проекта.** **Файл ▸ Сохранить проект** (Cmd+S) сохраняет
работу целиком в файл `.3dpaint` — как PSD: все слои с картами цвета,
шероховатости, металла и прозрачности, свойства слоёв, положение модели,
ракурс, материал и кисть. Внутри обычный zip. История отмены не хранится.

**ИИ красит модель (MCP).** В приложении раскраску можно поручить Claude или
любому другому клиенту MCP: «крыша красная черепица, стены бежевые, дверь
синяя». Включается в **Настройках**; команда `claude mcp add …` для терминала
показана там же. ИИ изучает части модели, заливает их цветом и поверхностью на
своём слое — деталью целиком (ботинок, напульсник), по части, высоте, рамке или указав точку на снимке, — находит мелкие
недокрашенные пятна и видит их с номерами на каркасе и на всей развёртке,
отменяет свои шаги и снимает виды, чтобы проверить себя. Сервер слушает только
`127.0.0.1`, пускает по ключу и по умолчанию выключен; каждая заливка ИИ —
обычный шаг отмены.

**Сохранил — открыл — красишь дальше.** Модель, сохранённая с покраской,
открывается с ней же — GLB или OBJ вместе с `.mtl` и `.png`. Несколько файлов
сохраняются в одну папку, место выбирается один раз.

**Положение модели.** При открытии модель всегда встаёт на пол в центр мира.
Где у неё перед, файл не сообщает — поэтому смотрите модели в лицо и жмёте
**«Это перед»** (или **«Это верх»** для моделей, лежащих на боку). Положение
запоминается для каждой модели, а в файл она уходит в исходных координатах.
...
**Языки.** Интерфейс целиком говорит на семи языках — русском, английском,
немецком, испанском, французском, нидерландском и украинском — и
...
Рабочая версия 0.7.0. Новое в 0.7.0: раскраска силами ИИ выросла из первой
версии в полный набор — ИИ видит модель отдельными деталями геометрии, красит
их целиком или указывает точку на снимке, примеряет заливку до покраски,
смотрит крупные планы и детали, закрытые другими, находит недокрашенные пятна,
отменяет свои шаги и работает по описи референса. Плотные модели открываются
с текстурой 2048, узкие треугольники больше не держат старый цвет, одежда
рисуется с изнанкой, у программы новый значок. Ещё не сделано:

- ИИ: мазок кистью по точкам поверхности, выбор кисти и материала из
  библиотеки.
